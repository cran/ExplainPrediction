...
getPredictions<-function(model, instances, nLaplace=1, noClasses=2,classIdx=1) {
  ...
  if (model$noClasses == 0) { 
    # regression
    pCX <- predict(model, instances)
  }
  else{ 
    if (model$model == "svm")
      pred <- predict(model, instances, probability = TRUE)
    else if (model$model == "randomForest" )
      pred <- predict(model, instances, type="prob")
    else 
      pred <- predict(model, instances)
    
    if (is.matrix(pred)) { # nnet and randomForest
      if (ncol(pred)==1) {
        if (classIdx == 1)
          pCX <- pred[,1]
        else
                     ...
                     showNumbers = FALSE, recall=NULL) 
{
  ...
  if (visLevel %in% c("both","model")) {
    ...
    modelAVexplain(modelTitleName, displayAttributes, explainInfo$avNames[matched],  
                   ...
                   modelVisCompact=modelVisCompact, displayThreshold=displayThreshold, 
                   displayColor=displayColor, normalizeTo=normalizeTo, showNumbers=showNumbers)
  ...
  if (visLevel %in% c("both","instance")) {
    ...
    for (i in 1:noInst) {		
      ...
      explainVisPN(instanceTitle,problemName, row.names(testData)[i], modelName, className, classValueName, attrNames, 
                   ...
                   trueClass=trueClass[i], showNumbers=showNumbers)
    }
    if (fileType != "")
      dev.off()
  }
                         ...
                         showNumbers=FALSE)
{
  ...
  y = 1
  for(iA in 1:noAttr) {
    # plot the overall contribution of attribute
    if (!modelVisCompact)
    ...
    if (showNumbers) {
      labelTxt =sprintf("%.2f", xUp)
      if (xUp < xLim * 0.2) # plot to the left of the bar
        text(xUp+xLim/50, y+0.4*boxHeight, labels=labelTxt, adj=c(0, 0.5),col="black")
      else # plot within the bar
        text(xUp-xLim/50, y+0.4*boxHeight, labels=labelTxt, adj=c(1, 0.5), col="white")		
    }
    xDown <- attrExplainNeg[[iA]]
    xUp <- 0
    rect(xDown, y, xUp, y+0.80*boxHeight, col=colAttrNeg)
    if (showNumbers) {
      labelTxt =sprintf("%.2f", xDown)
      if (-xDown < xLim * 0.2) # plot to the left of the bar
        text(xDown-xLim/50, y+0.4*boxHeight, labels=labelTxt, adj=c(1, 0.5),col="black")
      else # plot within the bar
        text(xDown+xLim/50, y+0.4*boxHeight, labels=labelTxt, adj=c(0, 0.5), col="white")				
    }	
    y=y+1
    # plot attribute values contributions
    if (!modelVisCompact){
        ...
        if (usedValues[[iA]][v]){
          ...
          if (showNumbers) {
            ...
            if (xUp < xLim * 0.2) # plot to the left of the bar
              ...
              text(xUp-xLim/50, y+0.4*boxHeight, labels=labelTxt, adj=c(1, 0.5), col="black")		
          }
          ...
          if (showNumbers) {
            ...
            if (-xDown < xLim * 0.2) # plot to the left of the bar
              ...
              text(xDown+xLim/50, y+0.4*boxHeight, labels=labelTxt, adj=c(0, 0.5), col="black")				
          }	
          y=y+1
        }
      }
...
explainVisPN<-function(instanceTitle, problemName, instanceName, modelName, className, classValueName, attrNames, attrValues, 
                       ...
                       normalizeTo=0, trueClass="", showNumbers=FALSE)
{
  displayColor <- match.arg(displayColor)
  if (displayColor=="color"){
    colExplainPos = "blue"  
    ...
    for(iA in 1:noUsed) {
      ...
      if  (expl[usedAttr[[iA]]] >= 0.0) {
        ...
        if (showNumbers) {
          labelTxt =sprintf("%.2f", xUp)
          if (xUp < xLim * 0.2) # plot to the left of the bar
            text(xUp+xLim/50, y+boxHeight/8, labels=labelTxt, adj=c(0, 0.5),col="black")
          else # plot within the bar
            text(xUp-xLim/50, y+boxHeight/8, labels=labelTxt, adj=c(1, 0.5), col="white")		
        }
      }
      else {
        ...
        if (showNumbers) {
          labelTxt =sprintf("%.2f", xDown)
          if (-xDown < xLim * 0.2) # plot to the left of the bar
            text(xDown-xLim/50, y+boxHeight/8, labels=labelTxt, adj=c(1, 0.5),col="black")
          else  # plot within the bar
            text(xDown+xLim/50, y+boxHeight/8, labels=labelTxt, adj=c(0, 0.5), col="white")				
        }	
      }
